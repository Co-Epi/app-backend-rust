...
impl MemoMapper for MemoMapperImpl {
    ...
    fn to_report(memo: Memo) -> PublicReport {
      ...
      let earliest_symptom_time = extract(&bits, &Self::TIME_USER_INPUT_MAPPER, next)
        .value (|v| next += v);
      let cough_severity = extract(&bits, &Self::COUGH_SEVERITY_MAPPER, next)
        .value (|v| next += v);
      let fever_severity = extract(&bits, &Self::FEVER_SEVERITY_MAPPER, next)
        .value (|v| next += v);
      let breathlessness = extract(&bits, &Self::BOOLEAN_MAPPER, next)
        .value (|v| next += v);
...
fn extract<T>(bits: &Vec<bool>, mapper: &dyn BitMapper<T>, start: usize) -> ExtractResult<T> {
  ...
  let sub_bits: Vec<bool> = bits[start..end].try_into()
...
mod tests {
  use super::*;
  use crate::reporting::public_report::{FeverSeverity, UserInput, CoughSeverity};
